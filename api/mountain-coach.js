...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (profile?.tier !== 'pro') return res.status(403).json({ error: 'Pro required' });
    ...
    const aiRes = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({ model: 'anthropic/claude-sonnet-4-5', max_tokens: 1200, messages: [{ role: 'system', content: systemPrompt }, ...messages] }),
    });
    ...
    return res.status(200).json({ message: reply, resort: trailData?.resort ?? null });
  } catch (err) {
